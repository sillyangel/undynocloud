        ...
        start: function (connection, onSuccess, onFailed) {
                ...
                reconnectTimeout = window.setTimeout(function () {
                    if (opened === false) {
                        ...
                        if (connection.eventSource.readyState !== window.EventSource.OPEN) {
                            // If we were reconnecting, rather than doing initial connect, then try reconnect again
                            //DYKNOW CHANGE
                            $connection.triggerHandler(events.onError, [signalR._.transportError(signalR.resources.eventSourceError, connection.transport, new Error("reconnect timeout"))]);
                            //END DYKNOW CHANGE
                            that.reconnect(connection);
                        }
                    }
            ...
            connection.eventSource.addEventListener("error", function (e) {
                ...
                if (e.eventPhase === window.EventSource.CLOSED) {
                    ...
                    connection.eventSource.close();//ensure we do not autoconnect. chromeos will sometimes connect again which violates assumptions as we
                    //always need to change the URL as it said literally 3 lines above this. I dont know why this is so hard to understand
                    //END DYKNOW CHANGE
                    that.reconnect(connection);
                } else {
                    // connection error
